...
void Game::DrawUI()
{
    ...
    // Renderで計算済みの行列を使う（DRY原則）
    DirectX::XMMATRIX view = m_cachedView;
    DirectX::XMMATRIX proj = m_cachedProjection;

    // =============================================
    //  グローリーキル「F」プロンプト表示
    // =============================================
    if (m_gloryKillTargetEnemy && m_fPromptTexture && m_spriteBatch && m_states)
    {
        // スクリーンサイズ取得
        RECT clientRect;
        ...
        // 敵の頭上にプロンプトを配置
        DirectX::XMFLOAT3 promptWorldPos = m_gloryKillTargetEnemy->position;
        promptWorldPos.y += 1.0f;

        // 3D → スクリーン座標に変換
        DirectX::XMVECTOR worldPos = DirectX::XMLoadFloat3(&promptWorldPos);
        DirectX::XMVECTOR screenPos = DirectX::XMVector3Project(
            worldPos,
            0.0f, 0.0f, screenW, screenH, 0.0f, 1.0f,
        ...
        if (screen.z > 0.0f && screen.z < 1.0f)
        {
            ...
            float dx = promptWorldPos.x - pPos.x;
            float dz = promptWorldPos.z - pPos.z;
            float dist = sqrtf(dx * dx + dz * dz);

            constexpr float PROMPT_BASE_SIZE = 100.0f;
            constexpr float PROMPT_MIN_SIZE = 48.0f;
            float promptSize = PROMPT_BASE_SIZE / (dist * 0.15f + 1.0f);
            if (promptSize < PROMPT_MIN_SIZE) promptSize = PROMPT_MIN_SIZE;

            // 脈動アニメーション
            float pulse = 1.0f + 0.08f * sinf(m_gameTime * 4.0f);
            promptSize *= pulse;

            float halfSize = promptSize * 0.5f;
            RECT destRect = {
                (LONG)(screen.x - halfSize),
                (LONG)(screen.y - halfSize),
                (LONG)(screen.x + halfSize),
                (LONG)(screen.y + halfSize)
            };
            ...
            m_spriteBatch->Draw(m_fPromptTexture.Get(), destRect, DirectX::Colors::White);
