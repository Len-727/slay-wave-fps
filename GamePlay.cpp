        ...
        if (m_navGrid.IsReady())
        {
            ...
            for (auto& enemy : m_enemySystem->GetEnemies())
            {
                ...
                if (dist > 0.1f)
                {
                    ...
                    // 【Walk↔Run切り替え距離】プレイヤーとの距離がこれ以下なら Walk
                    // Tank は近距離でも Walk（Run アニメがないため常に Walk）
                    float runThreshold = 8.0f;  // デフォルト（NORMAL）
                    switch (enemy.type)
                    {
                    case EnemyType::NORMAL: runThreshold = 5.0f; break;  // 8m で Walk に切替
                    case EnemyType::RUNNER: runThreshold = 5.0f; break; // 12m で Walk（足が速いので遠めから歩く）
                    case EnemyType::TANK:   runThreshold = 0.0f; break;  // 常に Walk（Run アニメなし）
                    case EnemyType::MIDBOSS: runThreshold = 6.0f; break;
                    case EnemyType::BOSS:   runThreshold = 6.0f; break;
                    }
                    ...
                    // アニメーション設定
                    // Run アニメがないモデルは常に Walk を使う
                    // → T-Pose 防止（Tank は Run.fbx を読み込んでいない）
                    std::string moveAnim = "Walk";  // デフォルトは Walk
                    if (shouldRun)
                    {
                        // 該当モデルの Run アニメが存在するか確認してから切り替え
                        Model* modelForType = nullptr;
                        switch (enemy.type)
                        {
                        case EnemyType::NORMAL:  modelForType = m_enemyModel.get();  break;
                        case EnemyType::RUNNER:  modelForType = m_runnerModel.get(); break;
                        case EnemyType::TANK:    modelForType = m_tankModel.get();   break;
                        case EnemyType::MIDBOSS: modelForType = m_midBossModel.get(); break;
                        case EnemyType::BOSS:    modelForType = m_bossModel.get();   break;
                        }

                        if (modelForType && modelForType->HasAnimation("Run"))
                        {
                            moveAnim = "Run";  // Run アニメがある → Run
                        }
                        // Run がなければ moveAnim は "Walk" のまま
                    }
